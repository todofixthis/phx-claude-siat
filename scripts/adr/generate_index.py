...
a decision exists, where this reaches one who does not (ADR 013).
...
def generate(adr_dir: Path, repo_root: Path) -> int:
    ...
    for path in sorted(adr_dir.iterdir()):
        ...
        if claimant != path.name:
            problems.append(
                f"shares its number with {claimant}; renumber whichever number nothing "
                "cites yet, since a number already cited cannot move"
            )
        if problems:
...
def report_scoped_to(paths: list[str], adr_dir: Path, repo_root: Path) -> int:
    ...
    for path in sorted(adr_dir.iterdir()):
        ...
        if problems:
            print(f"Warning: {path.name} could not be read, so it binds nothing here",
                  file=sys.stderr)
            continue
