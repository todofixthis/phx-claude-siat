...
### Process
1. Run `uv run --project <this skill's directory> emoji-seed` **from inside the target repo** — substitute the base directory reported when this skill loaded. `--project` only resolves the seed generator's own dependencies; the command still shells out to `git log` in your shell's cwd for the off-limits list, so running it from anywhere else fails with `git log failed: fatal: not a git repository…`. This prints your **seed emoji** and the off-limits list — the emoji recent commits already used. The seed is off-limits as a final selection too; its role is to constrain the scene, not become the commit emoji
