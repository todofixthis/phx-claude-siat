...
### Process
1. Run `uv run --project <this skill's directory> emoji-seed` — substitute the base directory reported when this skill loaded. This prints your **seed emoji** and the off-limits list. The seed is off-limits as a final selection — its role is to constrain the scene, not become the commit emoji
