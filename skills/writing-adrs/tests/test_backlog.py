...
class DeriveScopeTests(BacklogDirTestCase):
    ...
    def test_skips_a_bare_directory_link_into_docs_adr(self):
        """A link naming the docs/adr directory itself, with no filename, is still excluded.

        Without the directory check running before this one, `resolved` stays `docs/adr`
        (no trailing slash) here, so the exclusion misses it and the item is falsely
        reported as concerning every file under `docs/adr/`.
        """
        (self.repo_root / "docs" / "adr").mkdir(parents=True)
        content = item_text(links="[ADR dir]: ../adr\n")
        self.assertEqual(derive_scope(content, self.repo_root), [])

    def test_skips_a_bare_directory_link_into_docs_backlog(self):
        """A link naming the docs/backlog directory itself is excluded the same way."""
        content = item_text(links="[backlog dir]: .\n")
        self.assertEqual(derive_scope(content, self.repo_root), [])
    ...
    def test_a_leading_slash_target_resolves_from_the_repo_root(self):
        """Root-relative Markdown (`/skills/x.py`) must resolve from the repo root.

        `BACKLOG_DIR / target` uses pathlib's `/` operator, which discards `BACKLOG_DIR`
        entirely once `target` is absolute — without special-casing this, the entry
        would survive as the literal string `/skills/writing-adrs/adr.py`, which
        `binding()`'s always-relative subjects can never match.
        """
        self.write_target("skills/writing-adrs/adr.py")
        content = item_text(links="[adr.py]: /skills/writing-adrs/adr.py\n")
        self.assertEqual(derive_scope(content, self.repo_root), ["skills/writing-adrs/adr.py"])

    def test_a_leading_slash_target_that_climbs_above_the_root_is_skipped(self):
        """A root-relative target can still climb out via `..`, and is skipped like any other."""
        content = item_text(links="[outside]: /../outside.py\n")
        self.assertEqual(derive_scope(content, self.repo_root), [])
...
class BindingTests(BacklogDirTestCase):
    ...
    Subject resolution (directory slashing, absolute paths, a path outside the root) is
    `resolve_subjects()`'s own concern, shared with `adr.py`'s `binding()` and covered by
    its `ResolveSubjectsTests` — kept here only as the one representative case below,
    asserting that it propagates.
    """
