...
rot to fix, so nothing here can fail a build the way `scope_problems()` does (ADR 030).
...
def derive_scope(content: str, root: Path) -> list[str]:
    ...
    for target in RE_LINK_TARGET.findall(content):
        if "://" in target:
            continue
        # A leading slash is root-relative Markdown some renderers support, not a
        # literal filesystem path — resolve it from the repo root. Left to `BACKLOG_DIR
        # / target`, pathlib's `/` operator discards `BACKLOG_DIR` outright once the
        # right side is absolute, producing an entry `binding()`'s always-relative
        # subjects could never match.
        if target.startswith("/"):
            combined = os.path.normpath(target.lstrip("/"))
        else:
            combined = os.path.normpath(str(BACKLOG_DIR / target))
        resolved = PurePosixPath(combined).as_posix()
        if resolved.startswith("../"):
            continue
        # Trailing-slash a directory target before the exclusion check below, so a bare
        # `docs/adr` (no filename) is recognised as `docs/adr/` and excluded the same
        # way a link to a file beneath it would be.
...
def binding(root: Path, paths: list[str]) -> list[Item]:
    ...
    from a file in hand to the items concerning it.
    """
