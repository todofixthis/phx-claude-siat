...
- **Number sequentially** — never reuse a number: giving a retired one to a new decision leaves every citation of it resolving to the wrong decision, and resolving quietly is what makes that unfindable. Renumbering is a different act, with its own workflow below.
...
- **Deletion is the failure to plan for, not staleness** — numbers are never reused, so an ageing citation still points into a chain. The one exception is a both-merged renumber (see Renumbering an ADR): the vacated number stays with the decision that kept it, so a citation missed there resolves to a different decision rather than dangling. But refactor the code and the comment goes with it, and where it was an `Archived` ADR's defence, nothing reports the loss, because the ADR is invisible by design.
