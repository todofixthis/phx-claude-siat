...
- **`status`** — `Accepted`, `Archived`, or `Superseded`. All three stay in the repo; the last two are excluded from `docs/adr/INDEX.md`, which is what an agent loads by default.
  ...
  - `Archived` — in force, but binds no future work, so carrying it costs attention it cannot repay: a question answered once with nothing following from it, or a stopgap that expires on its own — declining to change a repo's default branch, pinning a dependency around a vulnerability with no upstream fix. Never archive a decision someone could breach; a constraint belongs in the index however settled it feels. Set it at any time, including long after the ADR was written, when a decision stops binding. It is the only status that hides a live decision, so say in the ADR why it need not be in mind; nothing else will.
