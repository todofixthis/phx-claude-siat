...
- **`archived-because`** — one sentence naming the defence and where a breacher meets it, so whether and why an ADR left the index reads at a glance. Required when status is `Archived`; omit otherwise. One line, whichever defence applies:
  ...
  - `archived-because: AGENTS.md's ## Dependency freeze section forbids every option this ADR rejected, met before the session's first action; revisit-when is unset.`
...
the defence would cover a superset of what archiving removes. It does not: the plugin's
`SessionStart`/`SubagentStart` hook (`hooks/hooks.json`, running
`skills/writing-adrs/hook.py`) injects a note telling every session and subagent,
unconditionally, to read `docs/adr/INDEX.md` before proposing an architectural change — an
agent missing `AGENTS.md` reaches the index the same way one that has it does. That one
counterexample is all the refutation needs. `README.md` is not a second: it describes what
the hook regenerates without sending anyone to the index.
