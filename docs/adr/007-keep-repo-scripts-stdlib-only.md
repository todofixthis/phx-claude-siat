...
>
> **Discharged in part by [ADR 031][].** The repository root now carries a Python
...
project.~~ **Built by ADR 031.** PEP 723 suits a genuinely standalone one-off, not
the way in.
...
- ~~A second script hand-parses a grammar this repo does not define.~~ **Fired and
  discharged by ADR 031** for `scripts/frontmatter.py` — see the 2026-09-05 entry
...
- 2026-09-03: question 1 **fired**; question 2 still answers no. The trigger reads "a second
  ...
  the monorepo backlog item this repository carried at the time went on to become ADR 031.
...
- 2026-09-05: discharged by ADR 031, which built the root project this decision reserved
  for the trigger above — `scripts/` is now a uv workspace member, resolved against a shared
  root `uv.lock`. `scripts/frontmatter.py` itself is untouched: ADR 031 answers only whether
