...
> [ADR 031](../adr/031-adopt-a-uv-workspace-at-the-repository-root.md), which gave
...
`scripts/pyproject.toml` now exists (ADR 031) with `dependencies = []`; adding PyYAML
there is what this item spends.

## Why it is still worth doing

ADR 007's `revisit-when` named this exact condition and ADR 031 answered only the
