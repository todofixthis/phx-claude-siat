...
- **Task 3's first GREEN run reliably passed only after a fourth wording round.** Three drafting-time-only fixes (extending the worked example, an explicit cross-section rule clause, an explicit "Common mistake" anti-pattern) each closed part of the gap but never made code-symbol first-mention-only linking reliable — two of those rounds' verification runs failed in different ways (over-linked once, never-linked once), a pattern of general LLM inconsistency composing a long document rather than a single fixable wording defect. The fix that actually worked (commit `6dae48a`) instead added a bullet to the pre-existing, mandatory `### Pass 2: Conciseness pass` self-review step: scan the drafted ADR for any reference linked more than once and demote every occurrence after the first. Two fresh full-process verification runs (drafting + both review passes) then passed, including the specific heading-repeat case that broke every earlier attempt. Accepted design: enforcement for this rule now leans on the existing Pass 2 backstop rather than getting it right purely at drafting time — the same trade-off (no lint/tooling backstop, relies on the agent actually running Pass 2) that every other Pass 2 checklist item already carries, not a new risk.
- **Two follow-ups were flagged during Task 3's final review as out of this plan's scope, not fixed:** (1) the "Targets by type" bullet doesn't address code symbols defined *outside* the ADR's own repository (the two final verification runs picked two different, arguably-both-imperfect target formats for the same external symbol); (2) the first-mention-only rule is now stated in four places in the same section (bullet, worked example, "Common mistake" example, Pass 2 bullet) — arguably deliberate belt-and-suspenders reinforcement, but worth a future conciseness look. Neither affects this plan's Global Constraints or the RED/GREEN criteria Task 3 was scoped to verify.
