...
Each ADR's `scope` frontmatter names the paths it binds, so ask which decisions cover the
file you are about to change rather than reading the whole index:
`python3 -m scripts.adr.generate_index --for <path>...`. The pre-commit hook runs it over
staged paths and reports what it finds without blocking. It also reports `Archived`
decisions, which are in force but kept out of `INDEX.md` — this is the only place they
surface (ADR 013).
...
Write `ADR 007` in comments and docstrings; use the `docs/adr/007` path form in error
messages, where the reader is stopped and needs somewhere to go, and where a clause of
*why* is what makes the message actionable. A citation under `skills/` ships to users
