...
`INDEX.md` carries each decision's `scope` — the paths it binds — so work out which
decisions cover the files you are changing from the index you have already read (ADR 013).
The pre-commit hook reports the same thing for staged paths without blocking, and is the
only place `Archived` decisions surface, being in force but kept out of `INDEX.md`.
